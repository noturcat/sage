...
import { CreateDiscoverForm, createDiscoverSchema } from '@/schema/createDiscover.schema'
import type { ImageData } from '@/schema/createDiscover.schema'
...
import style from './CreateDiscoverPage.module.scss'

/**
 * **Comprehensive form page** for creating new discovers.
 *
 * Feature-rich form page that allows users to create detailed discovers with structured
 ...
 *
 * Notes:
 * - Multi-step form with comprehensive discover information fields.
 ...
 * - State management integration with CreateDiscoverStore.
 */
...
function CreateDiscoverPage() {
	...
	const { formData, setFormData, images, setImages, reset, _hasHydrated } = useCreateDiscoverStore()
	...
	const form = useForm<CreateDiscoverForm>({
	...
	useEffect(() => {
		if (formData && images.length === 0) {
			...
			setImages(allImages as ImageData[])
		}
	}, [formData, images.length, setImages])

	const onSubmit = (data: CreateDiscoverForm) => {
		const extractContent = (field: JSONContent[]): JSONContent[] => {
			return field || []
		}
		...
		router.push('/discover/create-discover/publish')
	}
			...
			<section className={style.container}>
				<div className={style.buttonsContainer}>
					...
					<div className={style.continueButtons}>
						<CustomButton className={style.previewButton} variant="outline" radius="full">
							Preview
						</CustomButton>
						<CustomButton
				...
				<Form {...form}>
					<form onSubmit={form.handleSubmit(onSubmit)} className={style.content}>
						<div className={style.discoverInfo}>
							...
							<RichTextEditorInput
								...
								form={form as UseFormReturn<CreateDiscoverForm>}
							/>
						</div>
					</form>
				</Form>
